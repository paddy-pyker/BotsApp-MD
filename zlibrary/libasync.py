...
async def request(url, jar=None):
    try:
        async with aiohttp.ClientSession(cookies=jar) as sess:
...
async def download_book(url, jar=None):
    fileName = ''.join(random.choices(string.ascii_lowercase + string.ascii_uppercase, k=10))
    try:
    	async with aiohttp.ClientSession(cookies=jar) as sess:
            logger.info("GET %s" % url)
            async with sess.get(url) as resp:
...
async def post(url, data):
    try:
        async with aiohttp.ClientSession(cookie_jar=aiohttp.CookieJar()) as sess:
...
class ListItem(dict):
    ...
    def __init__(self, request, domain ,jar=None):
        super().__init__()
        self.__r = request
        self.domain = domain
        self.JAR = jar
    ...
    async def fetch(self): 
        ...
        temp = '%s%s' % (self.domain, dl_link.get('href'))

        parsed['downloaded_book_location'] = await download_book(temp,self.JAR)
...
class ResultPaginator:
    ...
    def __init__(self, url: str, count: int, request: Callable, domain: str,jar=None):
        self.count = count
        self.__url = url
        self.__r = request
        self.domain = domain
    ...
    def parse_page(self, page):
        ...
        for idx, book in enumerate(book_list, start=1):
            js = ListItem(self.__r, self.domain,self.JAR)
...
class AsyncZlib:
    ...
    async def _r(self, url: str):
        if self.semaphore:
            async with self.__semaphore:
                return await request(url,self.cookies)
        else:
            return await request(url,self.cookies)
    ...
    async def login(self, email, password):
        ...
        self._jar = jar
        temp = jar._cookies.get("singlelogin.me")

        
        try:
            self.cookies = {
        	"remix_userid":temp["remix_userid"].value,
        	"remix_userkey":temp["remix_userkey"].value
            }
        except:
            pass
    ...
    async def search(self, q="", exact=False, from_year=None, to_year=None,
        ...
        paginator = ResultPaginator(url=payload, count=count, request=self._r, domain=self.domain,jar=self.cookies)        
